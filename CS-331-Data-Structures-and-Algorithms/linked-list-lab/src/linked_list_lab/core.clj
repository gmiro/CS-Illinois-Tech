...
(defn sizecount [x count]
 "Counting the size of the list"

 (cond (nil? x) count

 :else (sizecount (:cdr x) (+ count 1))))

(defn delete-aux [elt xx]
  "Delete first occurance of `elt` from `xx`"
  (cond (empty? xx) nil
        (= elt (:car xx)) (:cdr xx)
        :else  (Cons. (:car xx) (delete-aux elt (:cdr xx)))))

(defn delete [elt xx]
 (List. (delete-aux elt (:data xx))(sizecount (delete-aux elt (:data xx)) 0)))
...
;;      number of deletions.
(defn delete-all-aux [elt x]
 (cond (nil? x) x
 (= elt (:car x)) (delete-all-aux elt (:cdr x))
 :else (Cons. (:car x) (delete-all-aux elt (:cdr x)))))

(defn delete-all [elt xx]
  "Delete all occurrences of `elt` from `xx`."
  (cond (empty? xx) nil
       :else (List. (delete-all-aux elt (:data xx))(sizecount (delete-all-aux elt (:data xx)) 0))))
